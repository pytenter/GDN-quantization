...
COMPILER_ENV=/data01/user2/.conda/envs/spatialcoc
SGLANG_SRC=/data01/user2/src/sglang-v0.5.19/python
...
[[ "$PHYSICAL_GPU" == 0 || "$PHYSICAL_GPU" == 1 ]] || { echo "GPU must be 0 or 1" >&2; exit 2; }
[[ -f "$PATCH" ]] || { echo "missing patch: $PATCH" >&2; exit 2; }
if [[ -n "$FINAL_ROTATION" ]]; then
  [[ -f "$FINAL_ROTATION" ]] || { echo "missing final rotation: $FINAL_ROTATION" >&2; exit 2; }
fi
...
export TRITON_CACHE_DIR="/data01/user2/cache/recurrent_parity_gpu${PHYSICAL_GPU}/triton"
export TORCHINDUCTOR_CACHE_DIR="/data01/user2/cache/recurrent_parity_gpu${PHYSICAL_GPU}/torchinductor"
...
{"physical_gpu":$PHYSICAL_GPU,"port":$PORT,"mode":"$MODE","patch":"$PATCH","final_rotation":"$FINAL_ROTATION","context_length":262144,"tp_size":1,"dtype":"bfloat16","max_running_requests":1,"deterministic":true,"cuda_graph":false,"radix_cache":false}
EOF
