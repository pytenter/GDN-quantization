...
def capture_one(torch, model, tokenizer, patch, row, chunk_size, max_tokens, token_writer):
    ...
    with torch.inference_mode():
        prefill = model(input_ids=prompt_ids, attention_mask=attention, use_cache=True)
...
def command_capture(args):
    ...
    jdump(status_path, {"status": "STARTING", "shard": args.shard_index, "units": len(rows)})
    model, tokenizer = load_model(torch)
    start_time = time.time()
    completed = 0
    try:
        with gzip.open(token_path, "wt", newline="", encoding="utf-8") as tf, summary_path.open("w", encoding="utf-8") as sf:
            ...
            with BatchedMechanismPatch(torch, model) as patch:
                for row in rows:
                    ...
                    summaries = capture_one(torch, model, tokenizer, patch, row, args.chunk_size, args.max_tokens, writer)
                    for s in summaries:
